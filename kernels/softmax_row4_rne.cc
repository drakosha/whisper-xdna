...
void softmax_row4r_bf16(bfloat16 *restrict in, bfloat16 *restrict out,
                       int32_t n) {
  // Round to nearest even instead of the part's default truncation toward
  // zero. On the GELU kernel the same one line moved the bias from -2.0e-04 to
  // +4.3e-05 and end-to-end cosine from 0.99411 to 0.99462 at equal wall time
  // (measured); softmax rounds twice per element, so it should care too.
